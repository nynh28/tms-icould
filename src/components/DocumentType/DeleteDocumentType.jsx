...
import { useDeleteDocumentTypeMutation } from "../../services/apiSlice";
import toast from "react-hot-toast";

const DeleteDocumentType = ({ open, setOpen, deleteId }) => {
  const { t } = useTranslation();

  const [deleteQuery, { isLoading }] = useDeleteDocumentTypeMutation();

  const onDelete = async () => {
    try {
      await deleteQuery(deleteId);
      toast.success(t("message.success.delete", { field: t("DocumentType") }));
      setOpen(false);
    } catch (error) {
      console.log(error);
    }
          ...
          <div className="sm:flex sm:items-start">
            ...
            <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              <h3 className="text-lg font-medium leading-6 text-gray-900">
                {t("deleteTitle", { field: t("DocumentType") })}{" "}
                <span className="font-semibold text-red-500">{deleteId}</span>
              </h3>
              <div className="mt-2">
                <p className="text-sm text-gray-500">
                  {t("deleteSubtitle", { name: t("DocumentType") })}
                </p>
              </div>
            </div>
          </div>
