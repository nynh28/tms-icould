...
import { useDeleteUnitMutation } from "../../services/apiSlice";
import toast from "react-hot-toast";

const DeleteUnit = ({ open, setOpen, deleteId }) => {
  const { t } = useTranslation();

  const [deleteQuery, { isLoading }] = useDeleteUnitMutation();

  const onDelete = async () => {
    try {
      await deleteQuery(deleteId);
      toast.success(t("message.success.delete", { field: t("Unit") }));
      setOpen(false);
    } catch (error) {
      console.log(error);
    }
