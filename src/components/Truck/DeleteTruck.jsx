...
import { useDeleteTruckMutation } from "../../services/apiSlice";
import toast from "react-hot-toast";

const DeleteTruck = ({ open, setOpen, deleteId }) => {
  const { t } = useTranslation();

  const [deleteQuery, { isLoading }] = useDeleteTruckMutation();

  const onDelete = async () => {
    try {
      await deleteQuery(deleteId);
      toast.success(t("message.success.delete", { field: t("Truck") }));
      setOpen(false);
    } catch (error) {
      console.log(error);
    }
