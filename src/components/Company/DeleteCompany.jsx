...
import { useDeleteRttCompanyMutation } from "../../services/apiSlice";
import toast from "react-hot-toast";

const DeleteCompany = ({ open, setOpen, deleteId }) => {
  const { t } = useTranslation();

  const [deleteQuery, { isLoading }] = useDeleteRttCompanyMutation();

  const onDelete = async () => {
    try {
      await deleteQuery(deleteId);
      toast.success(t("message.success.delete", { field: t("Company") }));
      setOpen(false);
    } catch (error) {
      console.log(error);
    }
