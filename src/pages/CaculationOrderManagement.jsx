...
import FormCaculationOrder from "../components/CaculationOrder/FormCaculationOrder";
import DetailCaculationOrder from "../components/CaculationOrder/DetailCaculationOrder";
import FilterCaculationOrder from "../components/CaculationOrder/FilterCaculationOrder";
import { LoadingButton } from "@mui/lab";
import DeleteCaculationOrder from "../components/CaculationOrder/DeleteCaculationOrder";

const CaculationOrderManagement = () => {
    ...
    const { data, isLoading, isFetching, isSuccess, refetch } =
        useGetCaculationOrderQuery(criterias);
    ...
    const columns = [
        ...
        //       <Tooltip title={t("updateCaculationOrder")} placement="top-start" arrow>
        //             <a
                ...
                <div className="flex-1 transition-all duration-[300ms]">
                    ...
                    <div className="h-[calc(100vh_-_110px)] lg:mx-auto lg:max-w-full ">
                        <div className="flex h-full  min-h-[calc(100vh_-_110px)] bg-white ">
                            ...
                            <DeleteCaculationOrder open={open} setOpen={onDoneDelete} deleteId={selectedRow?.id} />
                ...
                <div className={(showDetail ? 'w-[600px]' : 'w-[0px]') + " transition-all duration-[300ms]  border-l"}>
                    ...
                    <div className="h-[calc(100vh_-_110px)] lg:mx-auto lg:max-w-full p-[16px] overflow-auto">
                        <div className="max-w-[700px] p-4 min-h-[50vh] bg-white border m-auto ">
                            {showDetail && (openEdit ? <FormCaculationOrder selectedItem={selectedRow} refetch={() => {refetch(); setOpenEdit(false)}} triggleSubmit={triggleSubmit} setTriggleSubmit={setTriggleSubmit} setOpenForm={setShowDetail} submitError={() => setTriggleSubmit(false)}  /> : <DetailCaculationOrder detailRow={selectedRow} />)}
                        </div>
                    </div>
                </div>
            </div>
            <FilterRightBar open={openFilter} setOpen={setOpenFilter} triggleFiter={triggleFiter} setTriggleFiter={setTriggleFiter}>
                <FilterCaculationOrder fleets={[]} filter={criterias} setFilter={updateFilter} triggleFiter={triggleFiter} setTriggleFiter={setTriggleFiter}/>
            </FilterRightBar>
            <FormDisplay open={openForm} setOpen={setOpenForm} >
                <FormCaculationOrder selectedItem={null} refetch={refetch} setTriggleSubmit={setTriggleSubmit} setOpenForm={setOpenForm} />
            </FormDisplay>
        </>
    );
};

export default CaculationOrderManagement;
