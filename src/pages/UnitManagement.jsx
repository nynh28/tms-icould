...
import { useGetUnitQuery } from "../services/apiSlice";
import FormUnit from "../components/Unit/FormUnit";
import DetailUnit from "../components/Unit/DetailUnit";
                ...
                <div className="flex-1 transition-all duration-[300ms]">
                    ...
                    <div className="h-[calc(100vh_-_110px)] lg:mx-auto lg:max-w-full ">
                        <div className="flex h-full  min-h-[calc(100vh_-_110px)] bg-white ">
                            ...
                            <DeleteUnit open={open} setOpen={onDoneDelete} deleteId={selectedRow?.id} />
                ...
                <div className={(showDetail ? 'w-[600px]' : 'w-[0px]') + " transition-all duration-[300ms]  border-l"}>
                    ...
                    <div className="h-[calc(100vh_-_110px)] lg:mx-auto lg:max-w-full p-[16px] overflow-auto">
                        <div className="max-w-[700px] p-4 min-h-[50vh] bg-white border m-auto ">
                            {showDetail && (openEdit ? <FormUnit selectedItem={selectedRow} refetch={() => {refetch(); setOpenEdit(false)}} triggleSubmit={triggleSubmit} setTriggleSubmit={setTriggleSubmit} setOpenForm={setShowDetail} submitError={() => setTriggleSubmit(false)}  /> : <DetailUnit detailRow={selectedRow} />)}
                        </div>
                    </div>
                </div>
            </div>
            <FilterRightBar open={openFilter} setOpen={setOpenFilter} triggleFiter={triggleFiter} setTriggleFiter={setTriggleFiter}>
                {/* <FilterUnit fleets={[]} filter={criterias} setFilter={updateFilter} triggleFiter={triggleFiter} setTriggleFiter={setTriggleFiter}/> */}
            </FilterRightBar>
            <FormDisplay open={openForm} setOpen={setOpenForm} >
                <FormUnit selectedItem={null} refetch={refetch} setTriggleSubmit={setTriggleSubmit} setOpenForm={setOpenForm} />
            </FormDisplay>
        </>
    );
};
